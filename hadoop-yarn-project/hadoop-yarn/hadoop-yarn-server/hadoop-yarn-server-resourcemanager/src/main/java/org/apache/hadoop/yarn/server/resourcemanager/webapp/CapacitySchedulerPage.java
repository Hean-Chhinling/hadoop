  ...
  protected void postHead(Page.HTML<__> html) {
    ...
    html.
            ...
            __(
                    ...
                    ".wb-col[style*='left: 250px'] { width: 400px !important; }",
                    ...
                    /* Hide the Header and the file icon*/
            ...
            __("document.addEventListener('DOMContentLoaded', function() {",
                    ...
                    "  function parseQueueHTML(rootDiv){",
                    "     function processQueueElement(element, parentQueue = null, seenQueues = new Set()){",
                    "       const queue = {",
                    "           Queue: '',",
                    "           usedQueue: '',",
                    "           subQueues: []",
                    ...
                    "        if (qElement) queue.Queue = qElement.textContent.trim();",
                    "",
                    "        // Prevent duplicate insertion queue name",
                    ...
                    "                  const subQueue = processQueueElement(li, queue, seenQueues);",
                    ...
                    "               if (th && td){",
                    "                   const keyText = th.textContent.trim().replace(/:$/, '');",
                    "                   const key = keyText.replace(/\\W+/g, '');",
                    "                   queueDetails[key] = td.textContent.trim();",
                    "               }",
                    "            });",
                    "        }",
                    "",
                    "        // Extract active user info if available",
                    "        const userTable = element.querySelector('table#userinfo');",
                    "        console.log('User table:', userTable);",
                    "        if (userTable){",
                    "           activeUsersInfo = {};",
                    "           const headers = Array.from(userTable.querySelectorAll('th')).map(th =>",
                    "               th.textContent.trim().replace(/\\s+/g, '')",
                    "           );",

                    "           Array.from(userTable.querySelectorAll('td')).forEach((td, i) => {",
                    "                   const key = headers[i].replace(/\\W+/g, '').replace(/^./, c => c.toLowerCase());",
                    "                   activeUsersInfo[key] = td.textContent.trim();",
                    "           });",
                    "        }",
                    "",
                    "        console.log('Queue Details: ', JSON.stringify(queueDetails, null, 2));",
                    "        console.log('Active Users Info: ', JSON.stringify(activeUsersInfo, null, 2));",
                    "",
                    "        // Only attach details to a leaf node",
                    "        if (queue.subQueues.length === 0){",
                    "           if (queueDetails) queue.queueDetails = queueDetails;",
                    "           if (activeUsersInfo) queue.activeUsersInfo = activeUsersInfo;",
                    "        }",
                    "",
                    "        return queue;",
                    "      }",
                    "",
                    "   return { queues: [processQueueElement(rootDiv, null, new Set())] };",
                    "  }",
                    "  let queueJson = parseQueueHTML(document.getElementById('cs'));",
                    "  console.log('Convert HTML to Queue JSON:', JSON.stringify(queueJson, null, 2));",
                    "",
                    "  function convertFullJsonToTreeData(queuesData){",
                    "     function processQueue(queueData){",
                    "         const treeData = {",
                    "            title: queueData.Queue,",
                    ...
                    "                 title: '',",
                    "                 queueName: `${queueData.Queue} Queue Status`, // Put Queue Percentage at the last column",
                    "                 classes: 'first-row-header'",
                    "             });",
                    "",
                    "             Object.entries(queueData.queueDetails).forEach(([key, value]) => {",
                    "                 const formattedTitle = key.replace(/([A-Z])/g, \" $1\").replace(/^./, str => str.toUpperCase());",
                    "                 treeData.children.push({ title: formattedTitle, value: value, classes: 'queue-table-row' });",
                    "             })",
                    "          }",
                    "",
                    "          if (queueData.activeUsersInfo){",
                    "             const activeUserHeader = {",
                    "                 title: 'Active Users Info',",
                    "                 children: [",
                    "                     {",
                    "                       title: 'User Name',",
                    "                       value: 'Max Resource',",
                    ...
                    "                       maxAMResource: 'Max AM Resource',",
                    "                       usedAMResource: 'Used AM Resource',",
                    "                       schedulableApps: 'Schedulable Apps',",
                    "                       nonSchedulableApps: 'Non Schedulable Apps',",
                    "                       classes: 'user-header-info'",
                    "                      },",
                    "                      {",
                    "                       title: queueData.activeUsersInfo.userName || '', // In case there is no active user, display the title empty",
                    "                       value: queueData.activeUsersInfo.maxResource,",
                    "                       weight: queueData.activeUsersInfo.weight,",
                    "                       usedResource: queueData.activeUsersInfo.usedResource,",
                    "                       maxAMResource: queueData.activeUsersInfo.maxAMResource,",
                    "                       usedAMResource: queueData.activeUsersInfo.usedAMResource,",
                    "                       schedulableApps: queueData.activeUsersInfo.schedulableApps,",
                    "                       nonSchedulableApps: queueData.activeUsersInfo.nonSchedulableApps,",
                    "                       classes: 'queue-table-row'",
                    "                      }",
                    "                 ]",
                    "             };",
                    "             treeData.children.push(activeUserHeader);",
                    "         }",
                    "",
                    "         // Recursively process subQueues",
                    "         if (queueData.subQueues && queueData.subQueues.length > 0){",
                    "            queueData.subQueues.forEach(subQueue => {",
                    "                treeData.children.push(processQueue(subQueue));",
                    "            });",
                    "         }",
                    "",
                    "         return treeData;",
                    "   }",
                    "",
                    "   return queuesData.queues.map(queueData => processQueue(queueData));",
                    "   }",
                    "   console.dir(convertFullJsonToTreeData(queueJson), {depth: null });",
                    "",
                    ...
                    "     source: convertFullJsonToTreeData(queueJson),",
                    "     columns: [",
                    "       { id: '*', title: 'Property', width: '400px' },",
                    "       { id: 'value', title: 'Value', width: '200px' },",
                    "       { id: 'weight', title: 'Weight', width: '100px' },",
                    "       { id: 'usedResource', title: 'Used Resource', width: '200px' },",
                    "       { id: 'maxAMResource', title: 'Max AM Resource', width: '200px' },",
                    "       { id: 'usedAMResource', title: 'Used AM Resource', width: '200px' },",
                    "       { id: 'schedulableApps', title: 'Schedulable Apps', width: '200px' },",
                    "       { id: 'nonSchedulableApps', title: 'Non-Schedulable', width: '200px' },",
                    "       { id: 'usedQueue', title: 'Used Queue', width: '200px' },",
                    "       { id: 'queueName', title: 'Queue Name', width: '200px' },",
                    "     ],",
                    "     scrollParent: document.getElementById('queue-wrapper'),",
                    "     animation: { expand: 200, collapse: 150 },",
                    "     expand: (e) => {",
                    "       const queueName = e.node.title;",
                    "       const urlParams = new URLSearchParams(window.location.search);",
                    "       let currentQueues = urlParams.get('openQueues') || '';",
                    "       let queues = currentQueues.split(',').filter(q => q);",
                    "",
                    "       if (e.node.isExpanded()){",
                    "           // Add expanded queue to the page url",
                    "           if(!queues.includes(queueName)){",
                    "               queues.push(queueName);",
                    "               urlParams.set('openQueues', queues.join(','));",
                    "           }",
                    "       } else {",
                    "         const index = queues.indexOf(queueName);",
                    "         // Delete collapse queue from the page url",
                    "         if (index !== -1){",
                    "             queues.splice(index); // Delete from the given queue name to the end of array",
                    "             if (queues.length > 0){",
                    "                 urlParams.set('openQueues', queues.join(','));",
                    "             } else {",
                    "                 urlParams.delete('openQueues');",
                    "             }",
                    "         }",
                    "       }",
                    "       window.history.replaceState({}, '', `?${urlParams.toString().replace(/%2C/g, ',')}`);",
                    "     },",
                    "",
                    "     render: function (e) {",
                    "       const node = e.node;",
                    ...
                    "               const usedQueueStr = node.data.usedQueue;",
                    ...
                    "   });",
                    "   const urlParams = new URLSearchParams(window.location.search);",
                    "   const openQueuesParam = urlParams.get('openQueues');",
                    "   const openQueues = openQueuesParam ? openQueuesParam.split(',') : [];",
                    "   wbTree.visit((node) => {",
