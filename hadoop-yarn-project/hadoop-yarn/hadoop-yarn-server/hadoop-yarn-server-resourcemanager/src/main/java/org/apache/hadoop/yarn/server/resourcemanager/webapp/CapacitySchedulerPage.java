  ...
  protected void postHead(Page.HTML<__> html) {
    html.
            ...
            __("#cs-tree { padding: 5px; font-family: Arial, sans-serif; font-size: 14px; }",
                    ...
                    ".wb-row:hover{ background-color: #ddd2d2 !important; }",
                    ...
                    /* Hide the Header and the file icon at the active user inf*/
                    ".wb-header, .wb-icon.bi-file-earmark, div.wb-row.wb-expanded.queue-table-row>span.wb-node.wb-col>i.wb-icon.bi.bi-folder2-open { display: none !important; }",
                    ...
                    "        var(--bg-base) calc(var(--show-dash) * var(--fill-percentage))",
                    "    );",
                    "}",
                    ".queue-header::before {",
                    "    content: '';",
                    ...
                    "        var(--bg-base) var(--fill-percentage)",
                    "    );",
            ...
            __("document.addEventListener('DOMContentLoaded', function() {",
                    ...
                    "       const queue = { name: '', usedQueue: '', backgroundColor: null, dashedWidth: null, fullWidth: null, subQueues: [] };",
                    "",
                    ...
                    "        }",
                    "",
                    "        // Extract active users info if available",
                    ...
                    "                   {",
                    "                       title: '',",
                    "                       queueName: `${queueData.name} Queue Status`,",
                    ...
                    "                      },",
                    "                      ...queueData.activeUsersInfo.map(user => ({",
                    "                         title: user.UserName || '', // In case there is no active user, display the title empty",
                    "                         value: user.MaxResource,",
                    "                         weight: user.Weight,",
                    "                         usedResource: user.UsedResource,",
                    "                         maxAMResource: user.MaxAMResource,",
                    "                         usedAMResource: user.UsedAMResource,",
                    "                         schedulableApps: user.SchedulableApps,",
                    "                         nonSchedulableApps: user.NonSchedulableApps,",
                    ...
                    "     columns: [",
                    "       { id: '*', title: 'Property', width: '500px' },",
                    "       { id: 'value', title: 'Value', width: '300px' },",
                    ...
                    "       { id: 'nonSchedulableApps', title: 'Non-Schedulable', width: '300px' },",
                    "       { id: 'usedQueue', title: 'Used Queue', width: '200px' },",
                    "       { id: 'queueName', title: 'Queue Name', width: '300px' },",
                    "     ],",
                    "     scrollParent: document.getElementById('queue-wrapper'),",
                    "     expand: ({ node }) => {",
                    "       const urlParams = new URLSearchParams(window.location.search);",
                    "       const queueName = node.title;",
                    "       let queues = (urlParams.get('openQueues') || '').split(',').filter(q => q);",
                    "",
                    "       if (node.isExpanded()){",
                    ...
                    "             const fullWidth = node.data.fullWidth === '0%' ? '80%' : node.data.fullWidth; // Fall back logic to set up maximum width (only 80%) if it is empty",
                    "",
                    "             node._rowElem.style.setProperty('--full-width', `${fullWidth}`); /* fullWidth Acts as the maximum capacity*/",
                    ...
                    "             const dashWidth = node.data.dashedWidth === '0%' ? '100%' : node.data.dashedWidth; // Fall back logic to set up maximum width if it is empty",
                    "",
                    "             node._rowElem.style.setProperty('--dash-width', `${dashWidth}`);",
                    ...
                    "         }",
                    "",
                    "         if (node.hasClass('queue-table-row') && node.data.backgroundColor){",
                    ...
                    "   const openQueues = new URLSearchParams(window.location.search)",
                    "       .get('openQueues')",
                    "       ?.split(',') || [];",
                    "   wbTree.visit((node) => {",
